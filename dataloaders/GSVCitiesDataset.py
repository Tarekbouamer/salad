...
class GSVCitiesDataset(Dataset):
    ...
    def __getdataframes(self):
        ...
        csv_path =self.dataset_dir + "/Dataframes" + f"/{self.cities[0]}.csv"
        ...
        for i in range(1, len(self.cities)):
            tmp_df = pd.read_csv(
                self.dataset_dir + "Dataframes/" + f"{self.cities[i]}.csv"
            )
    ...
    def __getitem__(self, index):
        ...
        for i, row in place.iterrows():
            ...
            img_path = self.dataset_dir + "/Images/" + row["city_id"] + "/" + img_name
